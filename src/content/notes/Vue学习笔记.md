...
watch 属性是组件的属性，和 el,data,methods 等属性类似。
...
`Vuex`使用的是单一状态树(single source of truth),这意味着每个应用将仅仅包含一个 store 对象——最为‘唯一数据源’（SSOT）,state 类似于组件中的 data，遵循相同的规则。
